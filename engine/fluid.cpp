...
PtrGMat doFluid(const PtrGMat initPMat, const MSize &s)
{
    ...
    for (short x = 0; x < s.w; x++)
    {
        for (short y = 0; y < s.h; y++)
        {
            ...
            const UShort lastNum = IM->mat[x][y].water;
...
void setMaxMinWater(PtrGMat pmat)
{
    ...
    int min = pmat->mat[0][0].water;
    int max = min;
    for (UINT64 y = 0; y < hCellsCount ; y++) {
        for (UINT64 x = 0; x < vCellsCount ; x++) {
            const auto& val = pmat->mat[y][x].water;
            if (val < min) min = val;
            else if (val > max) max = val;
        }
    }

    pmat->setMMWater(MaxMin(max, min));
}
